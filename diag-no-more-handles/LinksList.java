...
//	private LinksManager           linksManager ;
	...
	private final Image _imageArrowLeft ;
	private final Image _imageArrowRight ;
	private final Image _imageForeignKey ;
	private final Image _imageJoinTable ;
	
	private final Color _colorGray = PluginColors.rgb( 240, 240, 240);
...
//		this.linksManager = linksManager ;
		...
		//--- Image instances 
		_imageArrowLeft  = PluginImages.getImage(PluginImages.ARROW_LEFT) ;
		_imageArrowRight = PluginImages.getImage(PluginImages.ARROW_RIGHT) ;
		_imageForeignKey = PluginImages.getImage(PluginImages.FOREIGNKEY);
		_imageJoinTable  = PluginImages.getImage(PluginImages.JOINTABLE);
//		_imageArrowLeft  = null ;
//		_imageArrowRight = null ;
//		_imageForeignKey = null ;
//		_imageJoinTable  = null ;
	}
	...
	protected Composite addRow( Composite parent, Object item )
	{
		...
		// 1804 links !!!
		// Error at 699 
		// rowCount = 699 without colors, without Tooltip, without images
		// rowCount = 699 with colors
		// rowCount = 699 with colors and tooltips
		// rowCount = 699 with colors and tooltips and images
...
//			l.setText( link.getJavaFieldType() + "  " + link.getJavaFieldName() ) ;
		...
		//--- 2.2
		{
			...
			if ( link.isOwningSide() ) {
				...
				label.setImage( _imageArrowRight );
				label.setToolTipText("Owning side");
			}
			else {
				label.setText( "" );
...
//				label.setImage( PluginImages.getImage(PluginImages.ARROW_LEFT) );
				label.setImage( _imageArrowLeft );
			}
			else {
				label.setText( "" );
			}
...
//					label.setImage( PluginImages.getImage(PluginImages.FOREIGNKEY) );
					label.setImage( _imageForeignKey );
...
//					label.setImage( PluginImages.getImage(PluginImages.JOINTABLE) );
					label.setImage( _imageJoinTable );
